...
def similarity_threshold(similarity_method, similarity_method_name, similarity_method_name_short):
    ...
    for min_similarity_req in similarity_min_cutoffs:
        ...
        # Make predictions
        output_file_name = "mean_rating_"+similarity_method_name_short + str(min_similarity_req) + "_min_results.csv"
        overall_rmse, total_time, coverage = rec.make_predictions(rec.mean_item_rating_neighbors, output_file_name, users, item_ratings, item_count, user_count)
        print("\tResults saved in:\t", output_file_name)
        rmse_mean_item.append(overall_rmse)
        time_mean_item.append(total_time)
        coverage_mean_item.append(coverage)

    curve_label = "Mean Item Rating with "+similarity_method_name
    generate_graphs(similarity_min_cutoffs, rmse_mean_item, time_mean_item, coverage_mean_item, curve_label, similarity_method_name_short+"min_similarity_", "Minimum Similarity Between Neighbors")
    print("---------------------------------------------------------------------------")
    return
...
def k_nearest(similarity_method, similarity_method_name, similarity_method_name_short):
    ...
    for k in k_vals:
        ...
        output_file_name = "mean_rating_"+similarity_method_name_short + str(k) + "_closest_results.csv"
        # Make predictions
        overall_rmse, total_time, coverage = rec.make_predictions(rec.mean_item_rating_neighbors, output_file_name, users, item_ratings, item_count, user_count)
        print("\tResults saved in:\t", output_file_name)
        rmse_mean_item.append(overall_rmse)
        ...
        coverage_mean_item.append(coverage)
    curve_label = "Mean Item Rating with "+similarity_method_name
    generate_graphs(k_vals, rmse_mean_item, time_mean_item, coverage_mean_item, curve_label, similarity_method_name_short+"k_nearest_", "K Nearest Neighbors")
    print("---------------------------------------------------------------------------")
...
def top_percent(similarity_method, similarity_method_name, similarity_method_name_short):
    ...
    for p in percent:
        ...
        output_file_name = "mean_rating_"+similarity_method_name_short + str(p) + "_percent_results.csv"
        overall_rmse, total_time, coverage = rec.make_predictions(rec.mean_item_rating_neighbors, output_file_name, users, item_ratings, item_count, user_count)
        print("\tResults saved in:\t", output_file_name)
        rmse_mean_item.append(overall_rmse)
        time_mean_item.append(total_time)
        coverage_mean_item.append(coverage)
    curve_label = "Mean Item Rating with "+similarity_method_name
    generate_graphs(percent, rmse_mean_item, time_mean_item, coverage_mean_item, curve_label, similarity_method_name_short+"top_percent_", "Top N% of Neighbors")
    print("---------------------------------------------------------------------------")
